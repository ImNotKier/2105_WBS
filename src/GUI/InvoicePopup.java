...
public class InvoicePopup extends JDialog {
    ...
    public InvoicePopup(int billingID) {
        setTitle("Invoice Details");
        setModal(true);
        setSize(500, 700);
        setLocationRelativeTo(null);
        setDefaultCloseOperation(JDialog.DISPOSE_ON_CLOSE);
        fetchAndDisplayInvoice(billingID);
    }

    private void fetchAndDisplayInvoice(int billingID) {
        ...
        try (Connection con = DatabaseConnector.getConnection()) {
            ...
            String consumerQuery = "SELECT FirstName, LastName, Address, Email FROM consumerinfo WHERE SerialID = (SELECT SerialID FROM bill WHERE BillingID = ?)";
            ...
            String billingQuery = "SELECT BillingID, BillingAmount, DueDate FROM bill WHERE BillingID = ?";
            ...
            // Fetch Payment Information using SerialID from the bill table (since SerialID is related to BillingID)
            String paymentQuery = "SELECT AmountPaid, PaymentDate FROM ledger WHERE SerialID = (SELECT SerialID FROM bill WHERE BillingID = ?)";
            ...
            String meterQuery = "SELECT PresentReading, PreviousReading FROM watermeter WHERE MeterID = (SELECT MeterID FROM consumerinfo WHERE SerialID = (SELECT SerialID FROM bill WHERE BillingID = ?))";
            ...
            // Fetch Concessionaire Information using ConcessionaireID related to the MeterID of the bill's SerialID
            String concessionaireQuery = "SELECT ConcessionaireName, PricePerCubicMeter FROM concessionaire WHERE ConcessionaireID = (SELECT ConcessionaireID FROM watermeter WHERE MeterID = (SELECT MeterID FROM consumerinfo WHERE SerialID = (SELECT SerialID FROM bill WHERE BillingID = ?)))";
    ...
    public static void main(String[] args) {
        int sampleBillingID = 1;
        new InvoicePopup(sampleBillingID);
    }
}
